...
function TradingViewMarketChart({ token, large = false }: { token: Token; large?: boolean }) {
  ...
  if (mode === 'trend') return <div className="market-chart-shell">{toolbar}<MarketChart token={token} large={large} /></div>
  return <div className={`market-chart ${large ? 'large' : ''} ${token.change < 0 ? 'down' : ''} candles-active`} aria-label={`${token.name} TradingView candlestick chart`}>{toolbar}<div className="tradingview-chart" ref={chartRef} /></div>
}
